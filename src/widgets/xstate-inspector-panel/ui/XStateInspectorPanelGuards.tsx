...
export function XStateInspectorPanelGuards({
	...
	return (
		<>
			...
			<div className="grid grid-cols-1 gap-2 @min-[480px]/inspector:grid-cols-2">
				...
				<p className="sr-only col-span-full -mt-1 text-[10px] text-muted-foreground/80 @min-[320px]/inspector:not-sr-only">
					{INSPECTOR_COPY.GUARDS_DIRECTION_HINT}
				</p>
				{guardIndicators.map((guardIndicator) => (
					<div
						key={guardIndicator.id}
						className="flex items-start gap-2 rounded-md border border-panel-border/50 bg-background/75 px-2.5 py-1.5"
					>
						<span
						...
						<span className="sr-only ml-auto shrink-0 items-center rounded bg-panel px-1.5 py-0.5 text-[10px] font-medium whitespace-nowrap text-muted-foreground @min-[320px]/inspector:inline-flex">
