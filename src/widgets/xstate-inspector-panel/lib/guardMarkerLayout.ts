...
import type { InspectorFlowNodePosition } from "./index";
import { resolveGuardMarkerNodeClearance } from "./index";
...
export type GuardMarkerLayoutContext = {
	isDesktopLayout: boolean;
	isLandscape: boolean;
};

export type GuardMarkerEdgeLayoutInput = {
	sourceY: number;
	targetY: number;
	sourceX: number;
	targetX: number;
...
export type GuardMarkerArrow = {
	id: string;
	symbol: string;
	styles: { left: string; top: string; transform: string };
};

export type GuardMarkerEdgeLayoutResult = {
	markerSize: number;
	markerGap: number;
	hitAreaSize: number;
	markerButtonStyles: {
...
export const resolveCollisionHash = (collisionSeed: string): number => {
	let hash = 0;
...
export const computeGuardMarkerEdgeLayout = (
	input: GuardMarkerEdgeLayoutInput,
	layoutContext: GuardMarkerLayoutContext,
	...
	const { isDesktopLayout, isLandscape } = layoutContext;
