...
import type { MachineGraphEdge } from "../model/types";
...
type CreateMachineGraphEdgesInput = {
...
const normalizeTransitionConfig = (
	...
	if (Array.isArray(transitionConfig)) {
		return transitionConfig.filter(
			(candidate): candidate is TransitionConfig =>
				typeof candidate === "object" && candidate !== null,
		);
	}

	if (typeof transitionConfig === "object") {
		return [transitionConfig as TransitionConfig];
	}

	return [];
};

export const normalizeMachineGraphTransitions = normalizeTransitionConfig;

export const resolveMachineGraphTargetNodeId = (
	machine: AnyStateMachine,
	sourceNodeId: string,
	rawTarget: string,
): string => {
	...
	const machinePathPrefix = `${machine.id}.`;
...
export const collectMachineGraphGuardKeys = (guard: unknown): string[] => {
	if (!guard) {
		return [];
	}
	...
	const nestedGuardKeys = nestedGuards.flatMap(collectMachineGraphGuardKeys);
	const guardType =
...
const getGuardFromMachineConfig = (
	...
	const sourceTransitions = normalizeTransitionConfig(
		sourceConfig?.on
			? (sourceConfig.on as Record<string, unknown>)[edge.transition.eventType]
			: undefined,
	);
	const rootTransitions = normalizeTransitionConfig(
		machine.config.on
	...
	for (const transitionConfig of [...sourceTransitions, ...rootTransitions]) {
		...
		for (const targetValue of targetValues) {
			...
			const resolvedTarget = resolveMachineGraphTargetNodeId(
				machine,
				edge.source.id,
				targetValue,
			);
			...
			const guardKeys = collectMachineGraphGuardKeys(transitionConfig.guard);
...
export const resolveMachineGraphEdgeGuardLabel = ({
	machine,
	edge,
}: {
	machine: AnyStateMachine;
	...
	const guardKeys = collectMachineGraphGuardKeys(edge.transition.guard);
...
const toMachineGraphEdge = (
	...
	const guard = resolveMachineGraphEdgeGuardLabel({ machine, edge });
...
export const createMachineGraphEdges = ({
	machine,
}: CreateMachineGraphEdgesInput): {
