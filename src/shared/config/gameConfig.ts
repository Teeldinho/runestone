import { CAMERA_MODES } from "./cameraModesConfig";

export const ROOM_CONFIG = {
	WIDTH: 12,
	HEIGHT: 6,
	DEPTH: 12,
	WALL_THICKNESS: 0.35,
} as const;

export const CORRIDOR_CONFIG = {
	WIDTH: 4,
	HEIGHT: 4,
	DEPTH: 8,
} as const;

export const DAGRE_CONFIG = {
	DIRECTION: "TB",
	NODE_SEPARATION: 120,
	RANK_SEPARATION: 160,
	MARGIN_X: 32,
	MARGIN_Y: 32,
} as const;

export const PLAYER_CONFIG = {
	MAX_HP: 100,
	MOVE_SPEED: 4.8,
	SPRINT_SPEED: 6.8,
	ROTATION_SPEED: 8,
	JUMP_FORCE: 5,
	CAPSULE_RADIUS: 0.32,
	CAPSULE_HALF_HEIGHT: 0.6,
} as const;

export const ENEMY_CONFIG = {
	MAX_HP: 45,
	DETECTION_RADIUS: 4,
	ATTACK_RADIUS: 1.5,
	PATROL_SPEED: 1.5,
	CHASE_SPEED: 2.8,
	ATTACK_COOLDOWN_MS: 1200,
	ATTACK_DAMAGE: 8,
} as const;

export const TOP_DOWN_CAMERA_POLAR_ANGLE = Math.PI / 2;

export const TOP_DOWN_CAMERA_LOCKED_AZIMUTH_ANGLE = Math.PI;

export const RELATIVE_CAMERA_MODES = [
	CAMERA_MODES.THIRD_PERSON,
	CAMERA_MODES.TOP_DOWN,
	CAMERA_MODES.FIRST_PERSON,
	CAMERA_MODES.FREE_ORBITAL,
] as const;

export const CAMERA_CONFIG = {
	THIRD_PERSON: {
		FOV: 65,
		OFFSET: [0, 2.2, -3.8],
		LERP_SPEED: 5,
		ORBIT: {
			MAX_DISTANCE: 8,
			MIN_DISTANCE: 2.4,
			MAX_POLAR_ANGLE: 1.508,
			MIN_POLAR_ANGLE: 0.2,
		},
	},
	TOP_DOWN: {
		FOV: 60,
		HEIGHT: 20,
		DISTANCE: 0,
		POLAR_ANGLE: TOP_DOWN_CAMERA_POLAR_ANGLE,
		LOCKED_AZIMUTH_ANGLE: TOP_DOWN_CAMERA_LOCKED_AZIMUTH_ANGLE,
		ORTHOGRAPHIC_ZOOM: 20,
		MIN_ZOOM: 12,
		MAX_ZOOM: 36,
		MIN_DISTANCE: 10,
		MAX_DISTANCE: 40,
		ZOOM_SPEED: 1.2,
	},
	FIRST_PERSON: {
		FOV: 78,
		HEAD_BOB_AMPLITUDE: 0.04,
		HEAD_BOB_FREQUENCY: 6,
	},
	FREE_ORBITAL: {
		FOV: 58,
		INITIAL_POSITION: [0, 16, -18],
		TARGET_OFFSET_Y: 1.25,
		MIN_DISTANCE: 5,
		MAX_DISTANCE: 28,
		ROTATE_SPEED: 0.8,
		PAN_SPEED: 0.8,
		ZOOM_SPEED: 1,
		MIN_POLAR_ANGLE: 0.1,
		MAX_POLAR_ANGLE: 1.508,
	},
} as const;

export const GAME_CANVAS_CONFIG = {
	CAMERA: {
		FOV: CAMERA_CONFIG.FREE_ORBITAL.FOV,
		POSITION: CAMERA_CONFIG.FREE_ORBITAL.INITIAL_POSITION,
		NEAR: 0.1,
		FAR: 120,
	},
	RENDERER: {
		DPR_RANGE: [0.75, 1.25] as const,
		SHADOWS_ENABLED: false,
	},
	UI: {
		CANVAS_HEIGHT_PX: 420,
	},
	SCENE: {
		FLOOR_SIZE: [21.6, 21.6] as const,
		FLOOR_OFFSET_Y: -0.2,
		FLOOR_ROTATION_X_RAD: -Math.PI / 2,
		FLOOR_ROUGHNESS: 0.92,
		FLOOR_METALNESS: 0.08,
		PILLAR_RADIUS: 0.35,
		PILLAR_HEIGHT: 2.8,
		PILLAR_POSITION_Y: 1.4,
		PILLAR_RADIAL_SEGMENTS: 18,
		PILLAR_ROUGHNESS: 0.84,
		PILLAR_METALNESS: 0.16,
		RUNE_ORB_RADIUS: 0.24,
		RUNE_ORB_HEIGHT: 1.6,
		RUNE_ORB_WIDTH_SEGMENTS: 20,
		RUNE_ORB_HEIGHT_SEGMENTS: 20,
		RUNE_EMISSIVE_INTENSITY: 1.6,
		GRID_SIZE: 24,
		GRID_DIVISIONS: 20,
		GRID_OFFSET_Y: 0.03,
		TORCH_LIGHT_DISTANCE: 14,
		TORCH_LIGHT_DECAY: 2,
		TORCH_POSITIONS: [
			[-4, 0, -4],
			[4, 0, -4],
			[-4, 0, 4],
			[4, 0, 4],
		] as const,
	},
	POSTPROCESSING: {
		BLOOM: {
			LUMINANCE_THRESHOLD: 0.92,
			LUMINANCE_SMOOTHING: 0.7,
			INTENSITY: 0.15,
			MIPMAP_BLUR: true,
		},
		VIGNETTE: {
			OFFSET: 0.3,
			DARKNESS: 0.3,
		},
		ENABLED_DEFAULT: false,
	} as const,
} as const;

export type DagreDirection = (typeof DAGRE_CONFIG)["DIRECTION"];
export type CameraConfig = typeof CAMERA_CONFIG;
export type GameCanvasConfig = typeof GAME_CANVAS_CONFIG;
